...
YOUR CATEGORY: Striking the 13th Amendment's exception clause
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/THIRTEENTH_EXCEPTION_CANDIDATE.md
...
- Source: REDDIT_THREAD_IDEAS.md §"NEW Categories" #2 (u/Hwttdzhwttdz). The doc's state ballot figures are explicitly "from memory — verify before use": Colorado Amendment A 2018 (~66%), Nebraska Amendment 1 2020 (~68%), Utah Amendment C 2020 (~80%), Alabama (2022 recompiled constitution, ~76%), Tennessee Amendment 3 2022 (~79%), Vermont Proposal 2 2022 (~89%), Oregon Measure 112 2022 (~55%), Nevada Question 4 2024 (~60%), California Prop 6 2024 (DEFEATED ~53% no — this cuts against; verify), Louisiana Amendment 7 2022 (defeated — confusing wording; verify). Verify every number on Ballotpedia / official state results and give the state's partisan lean for each — the red-state wide margins are the whole evidence case; the 2024 California defeat and Louisiana 2022 defeat are the counter-evidence.
- Federal record: the Abolition Amendment (S.J.Res. / H.J.Res. by Sen. Merkley / Rep. Williams, 116th–118th; reintroduced 2025?) — verify numbers and cosponsor party mix on congress.gov/GovTrack. National polling with party splits: search hard (YouGov, Data for Progress (advocacy), any 2020–2025 items on "prison labor" / "involuntary servitude exception") — likely thin; say so.
- Failure-mode / trap notes: operational impact on prison-work programs (Colorado post-2018 litigation — cite actual case status; the "paid vs voluntary labor" statutory fight); law-enforcement-coded opposition; is support for the state amendments about the principle or about the ballot wording? Existential test: report the argument neutrally (a "New Bill of Rights" completing the 13th's arc vs a symbolic change with statutory implementation). Symmetry: binds no party in particular — pass by construction; say so.
- Why-an-amendment: the exception clause is constitutional text — only an amendment removes it (strong pass). Recommendation: your honest read of the evidence.
- Candidate since: 2026-08-12.

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "thirteenth-exception",
 "file": "research/candidates/THIRTEENTH_EXCEPTION_CANDIDATE.md",
