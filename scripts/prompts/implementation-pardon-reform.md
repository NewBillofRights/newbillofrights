...
that backs it. Amendment slug: `pardon-reform`. Work only on this amendment.
...
2. `research/PARDON_RESEARCH.md` — the research memo for this amendment (already deep on federal/state law).
3. `site/src/content/amendments/pardon-reform.mdx` — the live page: its existing footnote keys, its
...
Append a new dated section to the END of `research/PARDON_RESEARCH.md`:
...
`site/src/content/amendments/pardon-reform.mdx` between "## Why it can pass" and "## What we left out":
...
- Every URL in a footnote must appear in `research/PARDON_RESEARCH.md` (which is why you append them there).
