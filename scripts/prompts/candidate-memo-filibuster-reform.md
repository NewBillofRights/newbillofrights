...
YOUR CATEGORY: Filibuster reform
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/FILIBUSTER_REFORM_CANDIDATE.md
...
- Corpus: Monmouth April 2021 (keep/reform/eliminate 38/38/16) and Jan 2022 (43/35/17); Economist/YouGov May 2021 keep 68% R / 32% D; Navigator Jan 2026 (D-aligned, flag) 51% of R say eliminating would be positive; NPR Nov 2025 on the demand to eliminate. Verify each on the source. This is the corpus's cleanest example of the incumbent/regime-coding failure mode — the memo's job is to SHOW the flip with dated numbers from both Senate configurations (2021–22 D majority vs 2025–26 R majority): find Monmouth / YouGov / Quinnipiac / Fox items from both periods.
- Framings: "keep vs eliminate"; "reform" (talking filibuster, 41-to-sustain, carve-outs); the Reddit inversion (u/smcstechtips: 67% required to KILL a bill); a constitutional supermajority-for-legislation clause vs. its opposite (constitutionalize simple-majority rule). Note the filibuster is a Senate rule (Art. I §5 rulemaking power) — the why-an-amendment test cuts both ways (only an amendment could permanently entrench or abolish it) — state that plainly.
- Existential test: is Senate procedure a threat to the future of the country, or a preference about outcomes? Report the argument each way in one paragraph without taking a side.
