...
that backs it. Amendment slug: `redistricting`. Work only on this amendment.
...
2. `research/REDISTRICTING_RESEARCH.md` — the research memo for this amendment (already deep on federal/state law).
3. `site/src/content/amendments/redistricting.mdx` — the live page: its existing footnote keys, its
...
Append a new dated section to the END of `research/REDISTRICTING_RESEARCH.md`:
...
`site/src/content/amendments/redistricting.mdx` between "## Why it can pass" and "## What we left out":
...
- Every URL in a footnote must appear in `research/REDISTRICTING_RESEARCH.md` (which is why you append them there).
