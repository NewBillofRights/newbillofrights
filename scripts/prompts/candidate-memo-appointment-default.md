...
YOUR CATEGORY: Appointment-default rule
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/APPOINTMENT_DEFAULT_CANDIDATE.md
...
- Source: REDDIT_THREAD_IDEAS.md §"NEW Categories" #3 (u/musashisamurai 90-day default confirmation; u/mypoliticalvoice deputies/deadlines; u/Ind132 knock-out fallback for blocked SCOTUS confirmations). CATEGORIES.md: symmetric by construction; answers the Garland-scenario gap in the Term Limits staggering design; likely rides as a companion clause in Target 2 — read TERM_LIMITS_RESEARCH.md's discussion of confirmation refusal / staggering trap and say precisely how this clause would slot in.
- Evidence: unpolled as a mechanism. Search for anything: polling on the 2016 Garland blockade and 2020 Barrett confirmation with party splits (Pew, Gallup, Marquette, YouGov, Monmouth 2016/2020 — "should the Senate hold hearings/vote") — these show the regime-coding trap directly (each party's position flipped between 2016 and 2020 — show it with dated numbers); polling on the confirmation process generally (Marquette items on the Court's confirmation process 2020–2025). Legislative/academic proposals: any deemed-confirmed proposals (e.g., statutory 90-day-vote proposals; scholarship on default rules — cite only what you fetch); vacancy statistics for executive/judicial posts (Partnership for Public Service political-appointee tracker; Federal Judicial Center / uscourts.gov vacancies) as context.
- Failure modes: forward-looking, both-parties wording is essential ("any president / any Senate"); note the trap that a default rule changes the balance of Art. II §2 advice-and-consent (blast radius: executive nominees, ambassadors, all Article III judges) — the memo should scope variants (SCOTUS only vs all nominations; deemed-confirmed vs forced floor vote vs recess-style interim). Why-an-amendment: strong pass (Art. II §2 text; Senate rules alone cannot bind future Senates on constitutional consent). Existential test: gridlock in confirmations vs a procedural preference — argue neutrally. Recommendation likely "fold into Target 2 as companion clause, pending a poll" — justify or dissent.
- Candidate since: 2026-08-12.

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "appointment-default",
 "file": "research/candidates/APPOINTMENT_DEFAULT_CANDIDATE.md",
