You are verifying an outside reviewer's notes on candidate-category research memos for the New Bill of Rights Foundation, and applying only what you can verify. Repo: /Users/michaelhamilton/Documents/newbillofrights. Today is 2026-08-18. Read /Users/michaelhamilton/Documents/newbillofrights/research/SELECTION_METHOD.md §4–5 first so you can judge the reviewer's gate-logic notes.

YOUR MEMOS (edit only these files): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/AGENCY_INDEPENDENCE_CANDIDATE.md, /Users/michaelhamilton/Documents/newbillofrights/research/candidates/STATEHOOD_CANDIDATE.md, /Users/michaelhamilton/Documents/newbillofrights/research/candidates/FILIBUSTER_REFORM_CANDIDATE.md, /Users/michaelhamilton/Documents/newbillofrights/research/candidates/FAIR_DISTRICTING_CANDIDATE.md

THE REVIEWER'S ### AGE_LIMITS_CANDIDATE.md
1. **Missing Evidence** - The memo omits *Gregory v. Ashcroft*, 501 U.S. 452 (1991), the landmark Supreme Court decision holding that state mandatory retirement ages for judges violate neither the Age Discrimination in Employment Act nor the Fourteenth Amendment's Equal Protection Clause. This controlling precedent directly answers the constitutional objection raised in the nuances section. Source: Supreme Court of the United States. CONFIDENCE HIGH.

### EXECUTIVE_LAWFULNESS_CANDIDATE.md
1. **Partisan Tone** - The phrase "would have blocked Biden-era spending beyond appropriations" names a specific officeholder in prose as an abuse example, violating the neutrality instructions. Source: Reviewer instructions. CONFIDENCE HIGH.

### DOJ_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### CIVIL_SERVICE_CANDIDATE.md
1. **Missing Evidence** - The memo omits the Supreme Court's First Amendment patronage cases (*Elrod v. Burns*, 427 U.S. 347 (1976), and *Branti v. Finkel*, 445 U.S. 507 (1980)), which established a constitutional bar against firing non-policymaking government employees for political reasons. This existing constitutional protection directly impacts the "why-an-amendment" analysis, as the Constitution is not silent on protecting career civil servants. Source: Supreme Court of the United States. CONFIDENCE HIGH.
2. **Partisan Tone** - The phrase "how durable democracies have been hollowed out from inside" reads as a view on the merits and uses dramatic, side-coded vocabulary to characterize the stakes of the policy dispute. Source: Reviewer instructions. CONFIDENCE HIGH.

### AGENCY_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### STATEHOOD_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### FILIBUSTER_REFORM_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*


### FAIR_DISTRICTING_CANDIDATE.md

No issues found.

### PROPORTIONAL_REPRESENTATION_CANDIDATE.md

No issues found.

### FUSION_VOTING_CANDIDATE.md

1. **Factual Error**: The memo states that South Carolina enacted a ban on fusion voting in 2022, but South Carolina is one of the few states (alongside New York and Connecticut) that actively allows fusion voting. Source: State election laws / Ballotpedia. CONFIDENCE: HIGH.

### GOOD_GOVERNMENT_DATA_CANDIDATE.md

No issues found.

### HOUSE_EXPANSION_CANDIDATE.md

1. **Missing Evidence**: The memo claims that "The only live federal vehicle found is H.J.Res. 186," missing the REAL House Act (e.g., H.R. 622 in the 118th Congress, introduced by Rep. Earl Blumenauer), which explicitly proposes expanding the House to 585 members. Source: Congress.gov (H.R. 622, 118th Congress). CONFIDENCE: HIGH.

### THIRTEENTH_EXCEPTION_CANDIDATE.md

No issues found.

### APPOINTMENT_DEFAULT_CANDIDATE.md

1. **Factual Error**: The memo claims the Whitehouse-Booker bill (S. 3096, 118th Congress) "contains no provision for Senate inaction," but the bill explicitly includes a waiver provision stating that if the Senate fails to act within 120 days, it is deemed to have waived advice and consent, allowing the President to appoint the nominee. Source: Congress.gov (S. 3096, 118th Congress, Section 3). CONFIDENCE: HIGH.
 for your memos are below. The reviewer (Gemini) is good at spotting gaps and unreliable on facts — it has fabricated URLs and "corrections" before. **Nothing enters a memo unless you fetched a page that supports it.** Load web tools with ToolSearch "select:WebSearch,WebFetch".

For each note:
1. **Read the memo passage it refers to** first; some notes misread the memo (e.g., a "factual error" that the memo does not actually assert). If the note misreads the memo, record "reviewer misread" and move on.
2. **Verify on the web** (2–3 attempts max per item): find the pollster's page, official record, court opinion, or established news source; fetch it; confirm it states the fact (numbers, dates, holdings). A URL from the reviewer is not verification.
3. **Apply:**
   - *Verified factual correction to the memo's own text* → fix the passage in place (these memos were drafted today and are not yet the founder's reviewed corpus), AND log it in the Review Additions section (below) as "Corrected: <what changed> — <source link>".
   - *Verified addition* (missing evidence, counterargument, poll, case) → append under a new final section of the memo:
       ## Review Additions (2026-08-18)
       *Added after an outside completeness review; every item below was verified against the linked source before inclusion.*
     as concise, sourced bullets in the memo's style (party breakdowns + pollster + fieldwork date for polls; link every source). If an addition bears on the gate result or path back, say so in the bullet (it usually will not).
   - *Gate-logic notes* (the reviewer says a verdict, failure-mode call, non-polling-test result, or recommendation does not follow from the memo's evidence) → re-read the memo's Sections 2–5 against SELECTION_METHOD §4–5. If the reviewer is right, fix the reasoning in place (Sections 3–5 may be edited; keep the "Status in one line" consistent with any change and say so in the JSON). If the reviewer is wrong or the point is judgment the memo already states, record "reviewer disagreement, not applied" with one sentence why. Never change a recommendation on the reviewer's say-so alone — only on evidence you fetched or on a logic error you can name.
   - *Tone / partisan-coding notes* → if the reviewer is right that a phrase reads as a view on the merits, as characterizing a party unfairly, or as strategic cheerleading, fix the sentence in place with the plainest neutral wording (state the evidence and the test). If the phrase is a quotation of the founder's recorded decision, leave it but make sure it is marked as a quotation. Use judgment; do not sand down accurate analytical statements.
   - *Unverifiable* → do nothing to the memo; list it in your JSON.
4. Also add every URL you cite to the memo's "## 6. Sources" list (one line each) — a downstream integrity check requires every URL used on the site to appear in the memo.
5. Do not touch the memo's front matter, headings, or the "Status in one line" unless a verified correction or gate-logic fix requires it (then say so in the JSON).

Nonpartisan discipline: pair both parties' conduct or name neither; officeholder names in citations only; no "landslide" (say "wide margin"); no "populist / elites / establishment / woke / radical / weaponized".

REVIEWER'S ### AGE_LIMITS_CANDIDATE.md
1. **Missing Evidence** - The memo omits *Gregory v. Ashcroft*, 501 U.S. 452 (1991), the landmark Supreme Court decision holding that state mandatory retirement ages for judges violate neither the Age Discrimination in Employment Act nor the Fourteenth Amendment's Equal Protection Clause. This controlling precedent directly answers the constitutional objection raised in the nuances section. Source: Supreme Court of the United States. CONFIDENCE HIGH.

### EXECUTIVE_LAWFULNESS_CANDIDATE.md
1. **Partisan Tone** - The phrase "would have blocked Biden-era spending beyond appropriations" names a specific officeholder in prose as an abuse example, violating the neutrality instructions. Source: Reviewer instructions. CONFIDENCE HIGH.

### DOJ_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### CIVIL_SERVICE_CANDIDATE.md
1. **Missing Evidence** - The memo omits the Supreme Court's First Amendment patronage cases (*Elrod v. Burns*, 427 U.S. 347 (1976), and *Branti v. Finkel*, 445 U.S. 507 (1980)), which established a constitutional bar against firing non-policymaking government employees for political reasons. This existing constitutional protection directly impacts the "why-an-amendment" analysis, as the Constitution is not silent on protecting career civil servants. Source: Supreme Court of the United States. CONFIDENCE HIGH.
2. **Partisan Tone** - The phrase "how durable democracies have been hollowed out from inside" reads as a view on the merits and uses dramatic, side-coded vocabulary to characterize the stakes of the policy dispute. Source: Reviewer instructions. CONFIDENCE HIGH.

### AGENCY_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### STATEHOOD_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### FILIBUSTER_REFORM_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*


### FAIR_DISTRICTING_CANDIDATE.md

No issues found.

### PROPORTIONAL_REPRESENTATION_CANDIDATE.md

No issues found.

### FUSION_VOTING_CANDIDATE.md

1. **Factual Error**: The memo states that South Carolina enacted a ban on fusion voting in 2022, but South Carolina is one of the few states (alongside New York and Connecticut) that actively allows fusion voting. Source: State election laws / Ballotpedia. CONFIDENCE: HIGH.

### GOOD_GOVERNMENT_DATA_CANDIDATE.md

No issues found.

### HOUSE_EXPANSION_CANDIDATE.md

1. **Missing Evidence**: The memo claims that "The only live federal vehicle found is H.J.Res. 186," missing the REAL House Act (e.g., H.R. 622 in the 118th Congress, introduced by Rep. Earl Blumenauer), which explicitly proposes expanding the House to 585 members. Source: Congress.gov (H.R. 622, 118th Congress). CONFIDENCE: HIGH.

### THIRTEENTH_EXCEPTION_CANDIDATE.md

No issues found.

### APPOINTMENT_DEFAULT_CANDIDATE.md

1. **Factual Error**: The memo claims the Whitehouse-Booker bill (S. 3096, 118th Congress) "contains no provision for Senate inaction," but the bill explicitly includes a waiver provision stating that if the Senate fails to act within 120 days, it is deemed to have waived advice and consent, allowing the President to appoint the nominee. Source: Congress.gov (S. 3096, 118th Congress, Section 3). CONFIDENCE: HIGH.
:
### AGE_LIMITS_CANDIDATE.md
1. **Missing Evidence** - The memo omits *Gregory v. Ashcroft*, 501 U.S. 452 (1991), the landmark Supreme Court decision holding that state mandatory retirement ages for judges violate neither the Age Discrimination in Employment Act nor the Fourteenth Amendment's Equal Protection Clause. This controlling precedent directly answers the constitutional objection raised in the nuances section. Source: Supreme Court of the United States. CONFIDENCE HIGH.

### EXECUTIVE_LAWFULNESS_CANDIDATE.md
1. **Partisan Tone** - The phrase "would have blocked Biden-era spending beyond appropriations" names a specific officeholder in prose as an abuse example, violating the neutrality instructions. Source: Reviewer instructions. CONFIDENCE HIGH.

### DOJ_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### CIVIL_SERVICE_CANDIDATE.md
1. **Missing Evidence** - The memo omits the Supreme Court's First Amendment patronage cases (*Elrod v. Burns*, 427 U.S. 347 (1976), and *Branti v. Finkel*, 445 U.S. 507 (1980)), which established a constitutional bar against firing non-policymaking government employees for political reasons. This existing constitutional protection directly impacts the "why-an-amendment" analysis, as the Constitution is not silent on protecting career civil servants. Source: Supreme Court of the United States. CONFIDENCE HIGH.
2. **Partisan Tone** - The phrase "how durable democracies have been hollowed out from inside" reads as a view on the merits and uses dramatic, side-coded vocabulary to characterize the stakes of the policy dispute. Source: Reviewer instructions. CONFIDENCE HIGH.

### AGENCY_INDEPENDENCE_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### STATEHOOD_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*

### FILIBUSTER_REFORM_CANDIDATE.md
*(No errors, missing evidence, gate logic flaws, or partisan tone detected.)*


### FAIR_DISTRICTING_CANDIDATE.md

No issues found.

### PROPORTIONAL_REPRESENTATION_CANDIDATE.md

No issues found.

### FUSION_VOTING_CANDIDATE.md

1. **Factual Error**: The memo states that South Carolina enacted a ban on fusion voting in 2022, but South Carolina is one of the few states (alongside New York and Connecticut) that actively allows fusion voting. Source: State election laws / Ballotpedia. CONFIDENCE: HIGH.

### GOOD_GOVERNMENT_DATA_CANDIDATE.md

No issues found.

### HOUSE_EXPANSION_CANDIDATE.md

1. **Missing Evidence**: The memo claims that "The only live federal vehicle found is H.J.Res. 186," missing the REAL House Act (e.g., H.R. 622 in the 118th Congress, introduced by Rep. Earl Blumenauer), which explicitly proposes expanding the House to 585 members. Source: Congress.gov (H.R. 622, 118th Congress). CONFIDENCE: HIGH.

### THIRTEENTH_EXCEPTION_CANDIDATE.md

No issues found.

### APPOINTMENT_DEFAULT_CANDIDATE.md

1. **Factual Error**: The memo claims the Whitehouse-Booker bill (S. 3096, 118th Congress) "contains no provision for Senate inaction," but the bill explicitly includes a waiver provision stating that if the Senate fails to act within 120 days, it is deemed to have waived advice and consent, allowing the President to appoint the nominee. Source: Congress.gov (S. 3096, 118th Congress, Section 3). CONFIDENCE: HIGH.


Return ONLY this JSON:
{"memos": [{"file": "...", "applied": ["one line per correction/addition/logic/tone fix applied, with source"], "misread": ["reviewer notes that misread the memo"], "unverified": ["notes you could not verify and did not apply"], "disagreed": ["gate-logic notes you judged wrong, with one sentence why"], "gateImpact": "one sentence — did the verdict, floor, or recommendation change?"}]}


## Second job (after the reviewer's notes): retry the memo agents' unverified items with Firecrawl
The memo agents could not fetch some pages (403/404/paywall/JS). Firecrawl is now approved: load it with ToolSearch "select:mcp__firecrawl__firecrawl_scrape" (and "select:mcp__firecrawl__firecrawl_search" if useful) and retry every item listed for YOUR memos in the file /private/tmp/claude-501/-Users-michaelhamilton-Documents-newbillofrights/bd9cfc68-aa33-4a13-8c15-a05908057716/scratchpad/unverified-lists.md. If a retry verifies a figure that the memo hedged or omitted, add it as a sourced bullet under the memo's "## Review Additions (2026-08-18)" section (create the section if needed) and, if the memo's body says a figure "could not be verified" for that item, fix that sentence in place. Every URL you use goes into "## 6. Sources". If still unreachable after Firecrawl, list it in your JSON under stillUnreachable with the URL you tried and the error.

Return ONLY this JSON (extend the schema): {"memos": [{"file": "...", "applied": [...], "misread": [...], "unverified": [...], "disagreed": [...], "gateImpact": "...", "retriedVerified": ["item — source URL"], "stillUnreachable": ["item — URL tried — error"]}]}