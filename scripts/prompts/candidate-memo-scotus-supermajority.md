...
YOUR CATEGORY: Supermajority requirement for the Supreme Court to strike down federal laws
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/SCOTUS_SUPERMAJORITY_CANDIDATE.md
...
- Source: AMENDMENT_IDEAS.md "Candidate Ideas" — Alex MacGregor (text message, 2026-08-18): require a supermajority of justices (e.g., 6–3 or more) for the Supreme Court to strike down an existing federal law, since a statute has already been passed by both houses and signed; his framing note ("limiting the power of activist judges") and Michael Hamilton's response ("new and not popular yet — no polling with party breakdowns"). Filed as a candidate 2026-08-18. Read research/rejected/COURT_EXPANSION_REJECTED.md (adjacent rejected category — Court structure) and TERM_LIMITS_RESEARCH.md's judicial section (Target 2) and say precisely what overlaps and what is the remainder.
- Historical record to verify at the source: Progressive-era supermajority proposals — Sen. William Borah's 1923 bill requiring 7 of 9 justices to invalidate an act of Congress; Sen. Robert La Follette's 1924 platform (Congress could re-enact a law struck by the Court); the 1930s court-reform debate; recent scholarship on supermajority rules for judicial review (e.g., Jed Shugerman, Ryan Doerfler & Samuel Moyn "Democratizing the Supreme Court" 2021 Calif. L. Rev.; the Presidential Commission on the Supreme Court of the United States, Dec. 2021, Chapter 5 — "supermajority voting requirements" discussion; verify chapters/pages). State precedents that exist TODAY: Nebraska Const. art. V §2 (five of seven judges to hold a legislative act unconstitutional), North Dakota Const. art. VI §4 (four of five), and Ohio's former 1912–1968 rule (six of seven) — verify each constitution's text at the state legislature / official site, and find any evidence of how they have operated (cases where a majority found a law unconstitutional but the supermajority rule kept it in force). Any congressional bill or joint resolution proposing a supermajority rule (search congress.gov / GovTrack; if 403, say so).
- Polling: search hard for ANY national poll with a party breakdown on requiring a supermajority of the Court to strike down laws (Pew, Gallup, AP-NORC, Marquette, YouGov, Data for Progress (advocacy), the 2021 Presidential Commission's public comments are not polling). Likely unpolled — say so and give the closest proxies, labeled: Marquette Law School Poll items on the Court's power / whether Congress should be able to override decisions / judicial review; Pew/Gallup Court approval by party (regime-coded — show the flip 2020→2025); support for SCOTUS term limits (already in Target 2); AP-NORC 2022–2025 items on the Court having too much power. Report each with party splits, pollster, date, link.
- Failure modes: regime/incumbent coding is the central risk — a supermajority rule protects statutes against the current Court's majority, so which party favors it likely tracks who holds the Court and Congress; show that with the approval-by-party flip; note Alex's own framing works in opposite directions for each side ("activist judges" vs "protecting laws Congress passed") — framing dependency. Symmetry: neutral by construction over time, but not in the moment. Blast radius / traps: does the rule apply to state laws too (a 5–4 striking a state law under the 14th Amendment — e.g., Brown-era or Obergefell-era decisions would have needed 6)? Does it apply to executive actions? Rigidity trap (entrenches statutes against constitutional review; SELECTION_METHOD §6.3 blast-radius); does it interact with the term-limits staggering (18-year terms) in Target 2. Existential-problem test: argue neutrally. Why-an-amendment: strong pass (Art. III / Marbury; Congress cannot set the Court's voting rule by statute — or can it? Note the debate on whether Congress could do this by statute under its power over the Court's procedure — cite the 2021 Commission's discussion, which is the authoritative neutral source).
- Recommendation: honest read — likely "hold — unpolled; commission a forward-looking both-parties poll ('any future Court, any future law') with the state-precedent framing" or "reject on regime coding / rigidity trap"; justify.
- Candidate since: 2026-08-18.
