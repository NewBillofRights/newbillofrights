...
YOUR CATEGORY: Good-government data
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/GOOD_GOVERNMENT_DATA_CANDIDATE.md
...
- The idea (Michael Hamilton, inspired by Steve Ballmer / USAFacts): a constitutional footing for official statistics / public government data — accurate, timely, nonpartisan publication of what government takes in, spends, and does. Corpus proxies: USAFacts "State of the Facts" 2017 (90% data critical; 81% nonpartisan sourcing) and later AP-NORC waves; Pew via Brookings 2015 (66% open data improves accountability); Results for America (advocacy). All are proxies for a valence idea — say so. Search for anything mechanism-specific: trust in federal statistical agencies (Pew, AP-NORC 2024–2026 — e.g., trust in BLS/Census data after the 2025 BLS commissioner dismissal — find independent polling with party splits if it exists), Evidence Act 2018 (statute), the 2025–26 statistical-agency events as CONTEXT with citations (name no officeholders in prose).
- Existential-problem test is the deciding test per SELECTION_METHOD §5.1 ("parked … despite likely polling 70–90%"); civic-information/AI was REJECTED on the same test (read research/rejected/CIVIC_INFORMATION_REJECTED.md and distinguish this candidate from it precisely — official statistics integrity vs informing citizens). Why-an-amendment: what would a constitutional clause do that the Evidence Act / Paperwork Reduction Act / Census Clause do not? Regime-coding risk: trust in statistics flips with who is in power — show it if data exist.
- Section 5: specify a poll of a concrete mandate ("the Constitution should require the government to publish accurate, nonpartisan statistics on revenues, spending, population…") and note the honest risk that it polls high and still fails §5.1.
- Candidate since: 2026-08-06.

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "good-government-data",
 "file": "research/candidates/GOOD_GOVERNMENT_DATA_CANDIDATE.md",
