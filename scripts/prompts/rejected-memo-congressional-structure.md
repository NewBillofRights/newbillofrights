...
YOUR CATEGORY: Senate / Congressional Structure
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/rejected/CONGRESSIONAL_STRUCTURE_REJECTED.md
...
- The idea as filed: Ryan Ward named "the Senate" as a next-tier target (specifics not discussed); the Reddit thread mined in REDDIT_THREAD_IDEAS.md shows Senate proportionality/abolition drawing large partisan fights. Distinguish: (a) making Senate representation proportional to population; (b) abolishing the Senate; (c) repealing the 17th Amendment (coded right); (d) House expansion / apportionment — NOTE this last one is a CANDIDATE, not rejected, so mention it only to say it lives elsewhere; (e) filibuster — a candidate, not rejected.
- Evidence: Pew's 2023 report "How Americans View Proposals to Change the Political System" and its 2026 roundup include a Senate item ("change the way the Senate is structured so states with more people have more senators" or similar) — get the party breakdown from Pew's page. Any Gallup/YouGov Senate items. Structural bar: Article V's entrenchment clause ("no State, without its Consent, shall be deprived of its equal Suffrage in the Senate") — cite the constitutional text (archives.gov or constitution.congress.gov) and reputable scholarship on whether it can be amended at all.
- Failure modes: high-topline/low-floor (probably Weak floor); the why-an-amendment test is passed trivially (only an amendment could do it) but Article V entrenchment makes it a feasibility fail — say so. Path back: probably "none identified" for proportionality/abolition; note the House-expansion candidate as where the live energy is.

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "congressional-structure",
 "file": "research/rejected/CONGRESSIONAL_STRUCTURE_REJECTED.md",
