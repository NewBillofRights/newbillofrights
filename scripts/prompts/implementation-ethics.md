...
that backs it. Amendment slug: `ethics`. Work only on this amendment.
...
2. `research/ETHICS_RESEARCH.md` — the research memo for this amendment (already deep on federal/state law).
3. `site/src/content/amendments/ethics.mdx` — the live page: its existing footnote keys, its
...
Append a new dated section to the END of `research/ETHICS_RESEARCH.md`:
...
`site/src/content/amendments/ethics.mdx` between "## Why it can pass" and "## What we left out":
...
- Every URL in a footnote must appear in `research/ETHICS_RESEARCH.md` (which is why you append them there).
