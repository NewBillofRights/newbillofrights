...
YOUR CATEGORY: Voting Rights & Election Access
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/rejected/VOTING_RIGHTS_REJECTED.md
...
- The one-line reason is "access-vs-integrity framing splits cleanly along party lines." Test that carefully: Pew's election-policy battery (most recent, 2023–2026: photo ID, early in-person voting, automatic registration, same-day registration, no-excuse absentee/mail voting, felon voting restoration, making Election Day a holiday) has party breakdowns for each — get them from Pew's own report. Gallup (Oct 2022/2024 election-policy items) and AP-NORC too. Expect: photo ID clears with a strong D floor; early voting clears; universal mail voting and automatic registration split; a "right to vote" affirmative amendment (H.J.Res. proposals by Rep. Pocan/others — cite congress.gov) and the John Lewis VRAA / Freedom to Vote Act votes were party-line — cite the roll calls.
- The honest finding may be that individual ELEMENTS clear the bar (photo ID + early voting) while any PACKAGE either side would draft does not, and that a package combining an ID requirement with an access guarantee has never been polled as one proposal — that is the path back. Say exactly that if the evidence supports it.
- Contributor: Ryan Ward named it a next-tier target (AMENDMENT_IDEAS.md).
- Also check whether the 2025–2026 SAVE Act / proof-of-citizenship polling has party splits (independent pollsters only), and Newsweek/Cato/Morning Consult June 2026 "photo ID to vote 66%" (already in POLLING_RESEARCH.md caution list — verify party split if published).

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "voting-rights",
 "file": "research/rejected/VOTING_RIGHTS_REJECTED.md",
