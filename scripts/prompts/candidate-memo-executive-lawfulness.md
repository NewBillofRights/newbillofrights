...
YOUR CATEGORY: Stopping unlawful executive action
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/EXECUTIVE_LAWFULNESS_CANDIDATE.md
...
- Overlap discipline: Target 7 (Executive Compliance with SCOTUS) already covers court compliance (Marquette Nov 2025 84%, 78R/92D; Pew April 2025). Read EXECUTIVE_COMPLIANCE_RESEARCH.md's held-back / package sections and evaluate ONLY the remainder this candidate names: (1) power-of-the-purse dominance / impoundment ("spend what Congress appropriates"), (2) limits on executive privilege for oversight, (3) narrowing executive immunity (post Trump v. United States 2024). AMENDMENT_IDEAS.md "Executive-accountability drafts" (Michael Hamilton) lists the specific drafts (power of the purse, supermajority emergency authorization, executive privilege for oversight, narrowing immunity) — evaluate each as a sub-item.
- Corpus evidence: PRRI Aug–Sept 2025 (16% agree the president may ignore Congress/SCOTUS; 24R/14I/10D agree) — a ~76% R floor on the abstract principle; CBS/YouGov Oct 2025 (Congress, not the president, should have most authority over spending — majorities of all parties, percentages not published — verify whether CBS/YouGov later published them); Reuters/Ipsos Feb 2025 OMB freeze 34/62 (no party split); AP-NORC April 2025. Impoundment as a mechanism is UNPOLLED per the corpus — search hard (2025–26 rescissions/pocket-rescission fights produced polling? Pew, AP-NORC, Quinnipiac, Marquette items on "president must spend money Congress appropriated"). Immunity: polling on Trump v. United States (2024) — find independent items with party splits (e.g., Marquette July 2024, YouGov, AP-NORC) — these are regime-coded; show it.
- Symmetry test is the hard one (SELECTION_METHOD §5.2 uses this exact category as the example: "the same clause would have blocked Biden-era spending beyond appropriations and border-wall reprogramming — if it can't carry both, it drops to the statutory list"). Pair examples across administrations or use none; officeholder names in citations only.
- Why-an-amendment: Impoundment Control Act 1974 and GAO opinions are statutory; Trump v. United States is a constitutional holding — only an amendment narrows it. Recommendation may differ by sub-item (e.g., appropriations fidelity → fold into Target 7 if symmetric polling appears; immunity → hold, regime-coded); say so.
- Candidate since: 2026-08-06.

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "executive-lawfulness",
 "file": "research/candidates/EXECUTIVE_LAWFULNESS_CANDIDATE.md",
