...
PAGE (edit only this file): /Users/michaelhamilton/Documents/newbillofrights/site/src/content/candidates/fbi-doj-independence.mdx
MEMO (the only source of facts and URLs): /Users/michaelhamilton/Documents/newbillofrights/research/candidates/DOJ_INDEPENDENCE_CANDIDATE.md
...
## Do
- Compare every number, date, count, and claim on the page against the memo as it now stands. Where the memo changed, change the page (prose AND footnote), including the frontmatter `summary` and `gate` if the gate result or floor changed. SPECIFIC CHANGE Addition: Pew Aug 2025 FBI favorability by party — 48% fav / 36% unfav overall; 54% R fav / 34% unfav; 45% D fav / 39% unfav; Pew notes pre-2020 phone surveys found only modest partisan gaps on the FBI (i.e., newly opened). Diagnosis-side, confirms regime coding on a second institution — add one sentence in 'Where it stands' regime-coding if not present. Marquette Nov 2025 crosstab file is 404 but the release page carries the figures — no page change needed for that. for this page: Addition: Pew Aug 2025 FBI favorability by party — 48% fav / 36% unfav overall; 54% R fav / 34% unfav; 45% D fav / 39% unfav; Pew notes pre-2020 phone surveys found only modest partisan gaps on the FBI (i.e., newly opened). Diagnosis-side, confirms regime coding on a second institution — add one sentence in 'Where it stands' regime-coding if not present. Marquette Nov 2025 crosstab file is 404 but the release page carries the figures — no page change needed for that.
...
{"slug": "fbi-doj-independence", "changed": ["one line per change made"], "gateChanged": <true|false>, "summary": "<final frontmatter summary>", "gate": "<final frontmatter gate>", "words": <body word count>, "flags": ["anything the editor should look at"]}
