...
YOUR CATEGORY: Supreme Court Expansion
OUTPUT FILE (create it; touch nothing else): /Users/michaelhamilton/Documents/newbillofrights/research/rejected/COURT_EXPANSION_REJECTED.md
...
- Corpus: ~40% overall; 59% D / 18% R — YouGov July 2024 (URL in POLLING_RESEARCH.md); Navigator May 2026 (advocacy). Verify; add Marquette Law School Poll's court-expansion items (Marquette asks "increase the number of justices" periodically — get the latest with party breakdown from law.marquette.edu/poll), Pew (2023 political-system proposals report includes expansion? verify), and Fix the Court / Brennan aggregations (advocacy — flag).
- Contrast with what DID pass: SCOTUS term limits 73% (Marquette 2026) and ethics enforcement — those live in the Term Limits and Ethics targets; this memo should say plainly the court-reform demand was routed there.
- Failure modes: high-topline/low-floor; incumbent/regime coding (support tracks which party's appointees hold the majority — cite any time series or 2016–2020 comparison you can verify). Why-an-amendment note: expansion needs only a statute (Judiciary Act), so it also fails the why-an-amendment test unless the amendment FIXES the number at nine — and "fix the Court at nine justices" is a separate framing (proposed as an amendment by Republicans, e.g., "Keep Nine" S.J.Res./H.J.Res. — cite congress.gov) that may poll differently; find any party breakdown for it. Contributor: Ryan Ward ("expand the court dramatically").

## Return value (your final message must be ONLY this JSON, nothing else)

{"slug": "court-expansion",
 "file": "research/rejected/COURT_EXPANSION_REJECTED.md",
