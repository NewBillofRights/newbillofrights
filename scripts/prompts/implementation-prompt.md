...
that backs it. Amendment slug: `{{SLUG}}`. Work only on this amendment.
...
2. `research/{{MEMO}}` — the research memo for this amendment (already deep on federal/state law).
3. `site/src/content/amendments/{{SLUG}}.mdx` — the live page: its existing footnote keys, its
...
Append a new dated section to the END of `research/{{MEMO}}`:
...
`site/src/content/amendments/{{SLUG}}.mdx` between "## Why it can pass" and "## What we left out":
...
- Every URL in a footnote must appear in `research/{{MEMO}}` (which is why you append them there).
